...
# Assigning Shallow group
pe_3d_df <- pe_df %>%
  mutate(Shallow = case_when(
    Max_Depth < 30 ~ "Shallow",
    TRUE ~ "Not shallow"
  ))
...
shallow_3depths_df <- pe_3d_df %>%
  dplyr::select(c(all_of(c(physicochemical_params, sample_params, biological_params)), "Shallow")) %>%
...
shallow_3d_boxplots <- ggplot(shallow_3depths_df,
                              aes(x = Shallow, y = Value, fill = Shallow)) +
  ...
  scale_fill_manual(values = c(`Shallow` = "#2a9d8f", `Not shallow` = "#37474f")) +
  labs(
    fill = "Shallow"
  ) +
...
ggsave(plot = shallow_3d_boxplots, filename = "./figures/boxplots_shallow_3depths.svg", dpi = 800, width = 30, height = 12)

# Perform Kruskal-Wallis test for all parameters

kw_results_doggers_3d <- doggers_3depths_df %>%
  group_by(`parameters`) %>%
  kruskal_test(Value ~ Shallow) %>%
...
doggers_7m_df <- pe486_3d_df %>%
  ...
  dplyr::select(c(all_of(c(physicochemical_params, sample_params, biological_params, vp_params)), "Shallow")) %>%
...
ggsave(plot = doggers_7m_boxplots, filename = "./figures/boxplots_doggers_bank_7m.svg", dpi = 800, width = 15, height = 12)
